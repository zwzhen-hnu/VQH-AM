...
def process_csv(df, max_ddg_std=MAX_DDG_STD, fold2pdbs=FOLD2PDBS):
    ...
    for (pdb_chains, mutations), sdf in not_unique_df.groupby(['#Pdb', 'Mutation(s)_cleaned']):
        ...
        rows.append(_dict)
    not_unique_stat = pd.DataFrame(rows).set_index(['#Pdb', 'Mutation(s)_cleaned'],drop=False)
    ...
    pdbs = {'#Pdb':[]}
    Mutation = {'Mutation(s)_cleaned':[]}

    for item in aggr_data.index :
        pdbs['#Pdb'].append(item[0])
        Mutation['Mutation(s)_cleaned'].append(item[1])
    ...
    aggr_data['pdb_id'] = pdbs.apply(lambda x: x.str.split('_')[0])
    aggr_data['pdb_fname'] = aggr_data['pdb_id'] + '_Repair.pdb'
    aggr_data['chain_a'] = pdbs.apply(lambda x: x.str.split('_')[1])
    aggr_data['chain_b'] = pdbs.apply(lambda x: x.str.split('_')[2])
    aggr_data['mutation'] = Mutation
    print(aggr_data)
