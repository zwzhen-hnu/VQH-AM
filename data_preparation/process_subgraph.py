...
def read_atoms(file, chain1, chain2):
    ...
    interaction_atoms = identify_interaction_sites(atoms1, atoms2, atom_coords1, atom_coords2, threshold=5.0)
    atoms = []
    ajs = []
...
for i in tqdm(range(len(df))):
    ...
    pdb_file = df.loc[i, "pdb_id"] + '_Repair_' + str(df.loc[i, "file_name"]) + '.pdb'
    chain1 = df.loc[i, "chain_a"]
    chain2 = df.loc[i, "chain_b"]
    data_processing(pdb_file, chain1, chain2)

np.save("./rball.edges.npy", np.asarray(r_edge_list, dtype=object))
np.save("./knn.edges.npy", np.array(k_edge_list, dtype=object))
torch.save(node_list, "./nodes.pt")
